			...
			<form action="?action=thanhtoan" method="POST">
				<?php if (empty($_SESSION['sanpham'])) { ?>
					<div class="empty-cart">
						...
						<p>Hãy chọn một sản phẩm yêu thích để bắt đầu mua sắm.</p>
						<a class="btn" href="?action=cuahang1">Khám phá sản phẩm</a>
					</div>
				<?php } else { ?>
								...
								foreach ($_SESSION['sanpham'] as $key => $value) { ?> <!-- $key là id sp -->
									<tr>
										...
										<td class="cart_total" style="text-align: center;">
											<p class="cart_total_price"><?php $_SESSION['tongtien'] += $value['thanhtien'];
																		echo $value['thanhtien'] . '.000 USD'; ?></p>
										</td>
								...
								<td style="font-size: 20px; color:orange; font-weight: bold; padding: 10px;">
									Tổng Tiền: <?php
												if (isset($_SESSION['tongtien'])) {
													echo $_SESSION['tongtien'] . '.000 USD';
												} else {
													echo "";
												}
												?>
